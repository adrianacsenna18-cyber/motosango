...
import { getMotoSession } from "@/lib/moto-auth";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (session && ride.motorista_id !== session.user.id) {
      return NextResponse.json(
        { error: "Você não tem permissão para gerar resumo financeiro desta corrida." },
        { status: 403 },
      );
    }
    ...
    const summaryValues = getInitialSummaryValues(
